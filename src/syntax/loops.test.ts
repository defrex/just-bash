...
describe('Bash Syntax - Loops', () => {
  describe('for loops', () => {
    ...
    it('should execute multiple commands in body', async () => {
      const env = new BashEnv();
      const result = await env.exec('for i in 1 2; do echo start $i; echo end $i; done');
    ...
    it('should work with file operations', async () => {
      ...
      const result = await env.exec('for f in /file1.txt /file2.txt; do cat $f; done');
  ...
  describe('while loops', () => {
    it('should execute while condition is true', async () => {
      ...
      const result = await env.exec('while grep -q 0 /count.txt; do echo iteration; echo 1 > /count.txt; done');
    ...
    it('should handle multiple iterations', async () => {
      ...
      const result = await env.exec('while grep -q aaa /counter.txt; do echo loop; echo "bbb" > /counter.txt; done');
    ...
    it('should return exit code from last command in body', async () => {
      ...
      const result = await env.exec('while grep -q start /f.txt; do echo done > /f.txt; true; done');
      expect(result.exitCode).toBe(0);
    });
  });
  ...
  describe('until loops', () => {
    it('should execute until condition becomes true', async () => {
      ...
      const result = await env.exec('until grep -q 1 /flag.txt; do echo waiting; echo 1 > /flag.txt; done');
    ...
    it('should execute when condition is initially false', async () => {
      ...
      const result = await env.exec('until grep -q yes /check.txt; do echo step; echo yes > /check.txt; done');
  ...
  describe('nested loops', () => {
    it('should handle nested for loops', async () => {
      const env = new BashEnv();
      const result = await env.exec('for i in a b; do for j in 1 2; do echo $i$j; done; done');
    ...
    it('should handle for inside while', async () => {
      ...
      const result = await env.exec('while grep -q go /run.txt; do echo inner; echo stop > /run.txt; done');
  ...
  describe('loop syntax variations', () => {
    ...
    it('should handle while loop without semicolon before do', async () => {
      ...
      const result = await env.exec('while grep -q x /f.txt do echo found; echo y > /f.txt; done');
