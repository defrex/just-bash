...
describe('Parse Errors - Comparison Tests', () => {
  ...
  const runRealBash = (command: string): { stdout: string; stderr: string; exitCode: number } => {
    try {
  ...
  const runVirtualBash = async (command: string, files: Record<string, string> = {}): Promise<{ stdout: string; stderr: string; exitCode: number }> => {
  ...
  describe('file not found errors', () => {
    it('should return exit code 1 for cat on missing file', async () => {
      ...
      const virtualResult = await runVirtualBash('cat /nonexistent_file_12345.txt');
    ...
    it('should return exit code 1 for grep on missing file', async () => {
      const realResult = runRealBash('grep pattern /nonexistent_file_12345.txt');
      const virtualResult = await runVirtualBash('grep pattern /nonexistent_file_12345.txt');
  ...
  describe('for loop syntax', () => {
    it('should execute valid for loop', async () => {
      ...
      const virtualResult = await runVirtualBash('for i in a b c; do echo $i; done');
  ...
  describe('while loop syntax', () => {
    it('should execute while false (zero iterations)', async () => {
      ...
      const virtualResult = await runVirtualBash('while false; do echo loop; done');
  ...
  describe('until loop syntax', () => {
    it('should execute until true (zero iterations)', async () => {
      ...
      const virtualResult = await runVirtualBash('until true; do echo loop; done');
  ...
  describe('if statement syntax', () => {
    ...
    it('should execute else branch when condition false', async () => {
      const realResult = runRealBash('if false; then echo yes; else echo no; fi');
      const virtualResult = await runVirtualBash('if false; then echo yes; else echo no; fi');
    ...
    it('should execute elif branch', async () => {
      const realResult = runRealBash('if false; then echo 1; elif true; then echo 2; else echo 3; fi');
      const virtualResult = await runVirtualBash('if false; then echo 1; elif true; then echo 2; else echo 3; fi');
